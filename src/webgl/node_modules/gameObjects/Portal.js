...
function Portal(world, pos) {
	...
	this.onEnterFrame = function onEnterFrame() {
		if((!world.userHead) || (world.userHead.homeWorld != world)) {
			return;
		}
		var dist = world.userHead.camera.localToWorld(new three.Vector3()).sub(mesh.position).length();
		if(!world.userHead.insidePortal && dist < radius * 0.99) {
			world.userHead.insidePortal = true;
			console.log("inside " + this.name);
			var normalizedCrosshairPos = world.userHead.camera.worldToLocal(this.mesh.position);
			console.log(normalizedCrosshairPos);
			// debugger;
			onPlayerEnterSignal.dispatch(normalizedCrosshairPos.z > 0);
			console.log("distance " + dist);
		} else if(world.userHead.insidePortal && dist > radius * 1.01) {
			world.userHead.insidePortal = false;
