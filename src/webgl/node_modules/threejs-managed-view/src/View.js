...
function View(props) {
	...
	var scene = props.scene || new THREE.Scene();
	...
	this.renderPasses = [];
	if(!props.omitDefaultScene) {
		this.addRenderPass(scene, camera, this.renderer.autoClear, 0x7f7f7f);
	}
	this.setupResizing();
...
View.prototype = {
	...
	render: function (time) {
		...
		if (!this.skipRender) {
			for (var i = 0; i < this.renderPasses.length; i++) {
				...
				if(rp.preRender) rp.preRender(time);
				if(rp.clearColor !== undefined){
					this.renderer.setRenderTarget(rp.renderTarget);
					this.renderer.setClearColor(rp.clearColor, 1);
					this.renderer.clearColor();
				}
				this.renderer.render(rp.scene, rp.camera, rp.renderTarget);
				if(rp.postRender) rp.postRender();
			}
		}
	},

	clearRenderPasses: function(index = 0) {
		this.renderPasses.length = Math.min(this.renderPasses.length, index);
	},

	addRenderPass: function (scene, camera, clearColor, preRender, postRender, renderTarget) {
	...
	setCameraPerspective: function(w, h) {
		this.renderPasses.forEach(rp => {
			...
			if(this.fitAxis == "x") rp.camera.setLens(w, h);
